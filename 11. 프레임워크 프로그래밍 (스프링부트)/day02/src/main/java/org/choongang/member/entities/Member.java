...
public class Member extends BaseEntity { // 클래스 명이 엔티티 명과 동일
    ...
    @OneToMany(mappedBy = "member", cascade = {CascadeType.REMOVE, CascadeType.PERSIST}, orphanRemoval = true) // One = 지금 클래스 - Member | Many = BoardData | "member" = BoardData 의 Member member 의 member 부분 | 제약 조건 CASCADE ON DELETE 가 아님!! | orphanRemoval 쓰기 위해 반드시 필요 : PERSIST | orphanRemoval : 고아 객체 삭제

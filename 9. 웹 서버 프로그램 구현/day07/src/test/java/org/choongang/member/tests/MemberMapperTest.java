...
public class MemberMapperTest {
    ...
    @BeforeEach
    void init() {
        session = DBConn.getSession(false); // test 끝나면 rollback 할 예정 = 원상태로 복구
    ...
    void registerTest() {
        ...
        member.setPassword(faker.regexify("\\w{8,16}").toLowerCase()); // 8자리 이상 16자리 이하
        member.setUserName(faker.name().fullName());

        int result = mapper.register(member); // 등록 테스트
        assertEquals(1, result);

        long cnt = mapper.exist(member.getEmail()); // 등록 회원 있는지 존재 여부 테스트
        assertEquals(1L, cnt);

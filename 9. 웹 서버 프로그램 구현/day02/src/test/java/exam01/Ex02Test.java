...
public class Ex02Test {
    ...
    void init() {
        ...
        // mailer = mock(Mailer.class); <= @Mock private Mailer mailer
        faker = new Faker(Locale.ENGLISH);
        // request = mock(HttpServletRequest.class); <= @Mock private HttpServletRequest request
    ...
    void test1() {
        ...
        then(mailer).should(only()).send(email); // send 함수가 loginService.process 메서드에서 단 한 번 호출되는지 테스트
    } // should -> 적어도 1번 이상 호출

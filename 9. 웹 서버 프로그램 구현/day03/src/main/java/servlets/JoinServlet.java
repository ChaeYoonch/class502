...
public class JoinServlet extends HttpServlet { // get -> 회원가입 양식 | host -> 양식 작성해서 넘어온 데이터 처리

    @Override // 출력 = resp 가 담당
    public void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        resp.setContentType("text/html;charset=UTF-8"); // 네트워크에 setContentType 로 Content-Type 생성
        // resp.setCharacterEncoding("UTF-8"); 얘가 없어도 6.0 버전대 이므로 알아서 기본값으로 설정되어 있음

        PrintWriter out = resp.getWriter(); // html 형식으로 회원가입 틀 생성
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        req.setCharacterEncoding("UTF-8"); // 서블릿 4버전까지, 기본 유니코드 2바이트 형식, 서블릿 6버전부터는 기본 유니코드가 3바이트(UTF-8)
